/* Purch Ad Tag API V4.1. Copyright 2015 Purch, Inc.*/
	  		...
	  		if (targetingMap.hasOwnProperty(key)) {
	  			...
	  			if (value && tmntag_isArray(value)) {
	  				for (var i=0; i<value.length; i++) {
	  					value[i] = value[i].replace(/'|39;|&amp;|&|#|\r\n|\r|\n|\t|\f|\%0A|\"|\%22|\%5C|\%23|\%26|\%26|\%09/gm, "");
	  				}
	  				tmntag.g[key.replace(/'|&|#/g, "")]= value;
	  			} else if (value) {
	  				tmntag.g[key.replace(/'|&|#/g, "")]= value.replace(/'|39;|&amp;|&|#|\r\n|\r|\n|\t|\f|\%0A|\"|\%22|\%5C|\%23|\%26|\%26|\%09/gm, "");
	  			}
			}
		}
	} catch (exception) {
    ...
    if (origUnit) {
      ...
      for (var k in targetingData) {
        if (targetingData.hasOwnProperty(k) && targetingData[k]) {
          if (k==='_p') {
        	  slotClone.setTargeting(k, (1*targetingData[k])+1);
          } else {
...
tmntag.start = function () {
	...
	if (tmntag_async) {
		setTimeout(startPrivate, PREBID_TIMEOUT);
	} else {
		startPrivate();
	}
}
...
/* /Purch Ad Tag API V4.1 */
/* OpenX */
(function(){
openxID = '${PAGE:OPENX_ID}';
