...
if(searchTemplate.length==1){var layoutRegex=/^(.*)Layout$/,layout=parseClass(searchTemplate,layoutRegex,"unknown");
}var url=constructAjaxUrl(fromHash,currHash,missingSections,isBookmarkOrNewEmptyReq(),cqid,layout);
...
var parseClass=function(divName,regex,defaultVal){var className=divName.attr("class"),listRegex=/ /,matchName,i,len;
if(className){className=className.split(listRegex);
len=className.length;
for(i=0;
i<len;
i++){matchName=className[i].match(regex);
...
var constructAjaxUrl=function(fromHash,currHash,sections,includeRrid,cid,layout){var url=sUtils.constructAjaxUrl(fromHash,currHash,sections,false);
url+=moveAmabotPrerenderedIntoUrlParam();
if(handleHistoryPage()){url+="&infoOnly=1";
}if(includeRrid){url+=("&rrid="+sessionContext.initRid);
}if(cid){url+="&oqid="+cid;
}if(layout){url+="&atfLayout="+layout;
}return url;
};
...
},filter:defaultLRFilter},{mainSel:".s-refinements-on-top-value a",pRestric:"#refinementsOnTop",cond:function(){return true;
...
$("#refinementsOnTop .s-refinements-on-top-value a").each(function(){self.rewriteLinks($(this));
});
