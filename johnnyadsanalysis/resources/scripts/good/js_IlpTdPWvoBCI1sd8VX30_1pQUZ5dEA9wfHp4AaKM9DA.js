...
jQuery(document).ready(function() {
	...
	mediaElements.each(function() {
		...
		var photoSource = $this.find('.views-field-field-photo-asset img').attr("src");
		if ($this.find('.views-field-field-instagram-video').length < 1) {
			$this.addClass('ig-photo');
		} else {
			$this.addClass('ig-video');
			var videoSrc = $this.find('.views-field-field-instagram-video').text().trim();
			//if (Modernizr.mq("only screen and (min-width : 1023px)")) {
			if (!isTouchDevice()) {
				$this.find('.views-field-field-photo-asset img, .views-field-field-photo-asset .play').live('click', function() {
					jQuery(this).parents('.views-field-field-photo-asset').html('<video autoplay controls>' + '<source src="' + videoSrc + '" type="video/mp4"></source>' + '</video>');
					//jQuery(this).parents('.views-field-field-instagram-photo').find('video').get(0).play();
				});
			} else {
				$this.find('.views-field-field-photo-asset').html('<video poster="' + photoSource + '" autoplay controls>' + '<source src="' + videoSrc + '" type="video/mp4"></source>' + '</video>');
			}
