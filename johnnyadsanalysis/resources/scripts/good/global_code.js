...
s.linkInternalFilters="javascript:,rpa.com,honda.com,redesign.automobiles.honda.com,automobiles.honda.com";
...
//s.variableProvider ='DFA#1515466:v37=[["DFA-"+lis+"-"+lip+"-"+lastimp+"-"+lastimptime+"-"+lcs+"-"+lcp+"-"+lastclk+"-"+lastclktime]]';
...
function s_doPlugins(s) {
  ...
  //This line populates the campaign variable when "cmpid" is present in the query string of the url
  if(!s.campaign)
    s.campaign=s.getQueryParam('cmpid')

  /* Beginning of Efficient Frontier block */	
  if(s.getQueryParam('ef_id')){
    var ef_tmp=s.getQueryParam('ef_id')
    var arr=s.split(ef_tmp,':');
    if(ef_tmp.indexOf('s_')>-1||ef_tmp.indexOf('c_')>-1){
      //If we have "s_" or "c_" then get 34 characters after the second ":"		
      var efid=arr[2].substring(0,34);			
    }else{
      //If we DON'T have "s_" or "c_" then get 34 characters after the second ":"		
      var efid=arr[2].substring(0,32);
    }
  }
  
  //This line places the parsed id into the campaign variable.
  s.campaign=efid;
  /* end of Efficient Frontier block */	

  /* Silverpop integration */
  s.campaign=s.getQueryParam("SP_MID");
  s.eVar38=s.getQueryParam("PROGRAMID");
  s.eVar39=s.getQueryParam("CAMPAIGNCODE");
  s.eVar40=s.getQueryParam("OFFERCODE");
  s.eVar41=s.getQueryParam("CELLCODE");
  s.eVar42=s.getQueryParam("PID");
  s.eVar43=s.getQueryParam("SP_RID");
  s.eVar45=s.getQueryParam("LINKNAME");  
  /* End Silverpop */


  // 2015-03-13 Populate eVar48 with "CID"
  s.eVar48 = s.getQueryParam("CID");


  /* This line ensures that the campaign variable is only set one time per visit */
  s.campaign=s.getValOnce(s.campaign,'s_campaign',0)

  /* Redirect from sub domain such as accord.honda.com */	
  if(!s.prop29) {
  s.prop29=s.getQueryParam('from');
  s.eVar29=s.prop29;
  }

  // copy search type from prop9 to evar9
  if(s.prop9) s.eVar9=s.prop9;

  // Copy search term to eVar if prop8 is populated
  if(s.prop8){
  if(s.prop10=="0"||s.prop10=="zero"||s.prop10=="null") s.prop8=s.prop10+":"+s.prop8;
    s.prop8=s.prop8.toLowerCase()
    s.eVar8=s.prop8;
    /* Set de-duped onsite search event */
    var t_search=s.getValOnce(s.eVar8,'ev8',0)
    if(t_search)
      s.events=s.apl(s.events,'event9',',',1)
    }
    
  //Populates prop1 if "module" is present in the query string of the url
  if(!s.prop1) s.prop1=s.getQueryParam('MODULE');

  //Populates prop2 if "year" is present in the query string of the url
  if(!s.prop2) s.prop2=s.getQueryParam('YEAR');

  //Populates prop3 if "ecolor" is present in the query string of the url
  if(!s.prop3) s.prop3=s.getQueryParam('ECOLOR');

  //Populates prop4 if "icolor" is present in the query string of the url
  if(!s.prop4) s.prop4=s.getQueryParam('ICOLOR');

  //Populates prop5 if "model" is present in the query string of the url
  if(!s.prop5) s.prop5=s.getQueryParam('MODEL');

  //Populates products if "selected" is present in the query string of the url
  if(!s.products && s.getQueryParam('SELECTED')) s.products = ";" + s.getQueryParam('SELECTED').replace(/\,/gi,",;");

  // gets the visit number and places it in evar4. uses the getvisitnum plugin
  s.eVar4=s.getVisitNum();

  // returns the value "new" or "repeat" using the getnewrepeat plugin 
  s.eVar5=s.getNewRepeat();
  s.prop38=s.eVar5;

  //If the pageType variable has a value then pass that value into the temp variable "s_temp_p14" to be used a few lines down
  //If not and pageName does not have a value, then use the getPageName plugin to populate "s_temp_p14"
  //If the pagename does have a value, then pass that value to s_temp_p14
  if (s.pageType) var s_temp_p14=s.pageType;
  else if (!s.pageName || s.pageName=="") var s_temp_p14=s.getPageName();
  else var s_temp_p14=s.pageName;


  // if no event is set, then persist the value of the s_temp_p14 variable using the getandpersist plugin
  // Otherwise, populate prop14 with s_temp_p14
  if (typeof(s.events)=="undefined" || s.events.indexOf("event9")=="-1") s.getAndPersistValue(s_temp_p14,'s_temp_p14',0);
  else s.prop14=s.getAndPersistValue('','s_temp_p14',0);


  //PageName Cleanup, unencodes the pagename
...
/*
 * Partner Plugin: DFA Check 0.7 - Restrict DFA calls to once a visit, 
...
//The below variables are configuration variables for the visitorID cookie.
s.visitorNamespace="honda"
s.trackingServer="omt.honda.com"
s.trackingServerSecure="somt.honda.com"
s.dc=112
