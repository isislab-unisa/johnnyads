...
*/
	...
	if((document.location.href).indexOf("lettura.corriere.it") > -1){
		if($(".cerca-libri-new").length > 0){
			$(".cerca-libri-new").find("form").attr("action","http://libreriarizzoli.corriere.it/is-bin/INTERSHOP.enfinity/WFS/RCS-RCS_PhysicalShops-Site/it_IT/-/EUR/ViewSearch-Start");
			$(".cerca-libri-new").find("input[type=hidden]").remove();
			$(".cerca-libri-new").find("form").append('<input type="hidden" name="utm_source" value="Corriere"><input type="hidden" name="utm_medium" value="Corriere_cercalibri"><input type="hidden" name="utm_campaign" value="Corriere_cercalibri"><input type="hidden" value="ALL" name="SearchCategory">');
		}
	}
