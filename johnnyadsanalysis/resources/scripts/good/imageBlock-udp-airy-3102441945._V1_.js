...
utils.embedAiryForChromelessPlayback=function(holder,videoHash,serverSideConfig,thumbs,logger,listItem){function selectFirstThumbnail(){A.trigger("image-block-select-thumbnail",thumbs.find("img:first"));
A.trigger("image-block-first-thumbnail-selected");
}return airyInstance=Airy.embed({isShowFirstFrameWhenReadyEnabled:true,parentElement:holder[0],streamingUrls:videoHash.url,swfUrl:serverSideConfig.swfUrl,foresterMetadataParams:serverSideConfig.foresterMetadataParams,backgroundColor:"transparent",flashParams:{wmode:"transparent"},experiments:{supportChromelessPlayback:true},bindings:{canplay:function(){var rendererType=(holder.find("video").length>0)?"HTML5":"FLASH";
...
},error:function(){selectFirstThumbnail();
...
A.trigger("image-block-video-fetch-completed",color,null,true);
}}});
...
utils.getMaxDimensionsOfMainImage=function(){var dims;
var mainImgHeightPartitions=options.mainImageHeightPartitions;
var mainImgMaxSizes=options.mainImageMaxSizes;
if(mainImgHeightPartitions&&typeof mainImgHeightPartitions!==undefined&&mainImgMaxSizes&&typeof mainImgMaxSizes!==undefined){var partition=0;
var viewPortHeight=window.innerHeight||document.documentElement.clientHeight||document.body.clientHeight;
for(var i=0;
i<mainImgHeightPartitions.length;
i++){if(viewPortHeight>mainImgHeightPartitions[i]){partition=i+1;
}}if(mainImgMaxSizes.length>=partition){dims=mainImgMaxSizes[partition];
}}return dims;
};
...
P.when("A","jQuery").register("imageMagnifier",function(A,$){var ue=window.ue;
function hoverZoomBoundInDPX(useClickZoom,burjImageBlock){var b={left:Infinity,top:Infinity,right:-Infinity,bottom:-Infinity};
var lc=$("#leftCol"),cc=$("#centerCol"),ccHze=$("#hover-zoom-end");
if(burjImageBlock){b.top=lc.offset().top;
b.bottom=lc.outerHeight()+b.top;
b.right=lc.outerWidth()+lc.offset().left;
}else{b.top=cc.offset().top;
...
$("#zoomWindow").remove();
var cropWindowCSSAttributes={overflow:"hidden",position:"absolute",backgroundColor:"white",zIndex:options.zIndex};
if(options.isHoverZoomShawdowEnabled||options.isHoverZoomEnhancedShadowEnabled){cropWindowCSSAttributes["boxShadow"]="0 6px 8px 2px rgba(0,0,0,.25)";
cropWindowCSSAttributes["border"]="1px solid #949494";
cropWindowCSSAttributes["borderColor"]="rgb(148,148,148)";
}var cropWindow=$("<div id='zoomWindow' />").css(cropWindowCSSAttributes);
if(options.burjImageBlock){cropWindow.fadeIn(200);
...
if(options.location=="over"){if(options.isUDP){rect=hoverZoomBoundInDPX(options.useClickZoom,options.burjImageBlock);
...
if(options.isHoverZoomEnhancedShadowEnabled&&window.pageXOffset!==undefined){var rightPadding=18;
var bottomPadding=14;
var width=$(window).width();
rect.width=width-rect.left+window.pageXOffset-rightPadding;
rect.height=rect.height-bottomPadding;
...
if(options.burjImageBlock){zw.show=function(){cropWindow.fadeIn(200);
return zw;
};
zw.hide=function(){cropWindow.fadeOut(200);
return zw;
};
}else{zw.show=function(){cropWindow.show();
return zw;
};
...
}var startedTimerToDismissZoomWindow=false;
var dismissZoomWindowHandler;
...
}if(cursorAt[0]<boundRect[0]||cursorAt[1]<boundRect[1]||cursorAt[0]>=boundRect[2]||cursorAt[1]>=boundRect[3]){if(options.burjImageBlock){if(!startedTimerToDismissZoomWindow){startedTimerToDismissZoomWindow=true;
dismissZoomWindowHandler=setTimeout(function(){disengage(true);
},500);
}}else{if(!options.useClickZoom){disengage();
}}}else{if(options.burjImageBlock){if(startedTimerToDismissZoomWindow){startedTimerToDismissZoomWindow=false;
clearTimeout(dismissZoomWindowHandler);
}}updateDetailPosition();
...
}}var hoverZoomOverlay="softlinesHoverZoomOverlay:TotalCount";
var hoverZoomOverlayCounter=ue.count(hoverZoomOverlay)||0;
ue.count(hoverZoomOverlay,hoverZoomOverlayCounter+1);
...
var airyConfig;
var logger;
var showBiggerImmersiveView=false;
...
}this.iVController=function(e,data,imageBlockUtils,imageBlockLogger,bypassUseIV){if(e==null||data==null||imageBlockUtils==null){return;
}if(!ivUtils){ivUtils=imageBlockUtils;
}if(!isIvApplicable(data,bypassUseIV)){return;
}if(!logger){logger=imageBlockLogger;
}showBiggerImmersiveView=data.showBiggerImmersiveView;
if(showBiggerImmersiveView){ivWidthAdjustment=0;
ivHeightAdjustment=50;
ivThumbSectionHeightAdjustment=110;
...
if(ivCurrentAltImageIndexType=="image"||ivCurrentAltImageIndexType=="ingress"){ivCurrentImageSelected=altImageSelectedDataInfo.index;
...
function constructLeftColumn(){var imgContainerHeightAdjustment=showBiggerImmersiveView?10:15;
var floatDirection=showBiggerImmersiveView?"right":"left";
ivLeftCol=$('<div id="iv-first-column" style="float: '+floatDirection+"; width:"+($(window).width()-ivWidthAdjustment-330)+'px;height:100%; margin-right: 15px;"/>');
var imgContainer=$('<div width="100%" id="iv-large-image" style="overflow: hidden; width: 100%; height: '+($(window).height()-ivHeightAdjustment-imgContainerHeightAdjustment)+'px;align:center; position:relative; top:0px; left:0px;"/>');
...
}function constructRightColumn(){var rightCol=$('<div style="float: left; padding-left:30px;" id="iv-sec-column"></div>');
if(!showBiggerImmersiveView){rightCol=$('<div style="float: left; width: 235px;" id="iv-sec-column"></div>');
...
var widthAdjustment=showBiggerImmersiveView?250:330;
$("#iv-first-column").css({"width":(width-widthAdjustment)+"px"});
...
prefetchResources();
if(!showBiggerImmersiveView){populateMediaSelection();
}populateRightColumn();
addHandlers();
resizeIV();
};
...
var isIvApplicable=function(data,bypassUseIV){var currentSelectedColor=getCurrentSelectedColor(data);
if(data.colorImages&&data.colorImages[currentSelectedColor]&&data.colorImages[currentSelectedColor].length>0&&(data.useIV||bypassUseIV)){return true;
}return false;
};
...
}var popoverWidth=showBiggerImmersiveView?$(window).width():($(window).width()-ivWidthAdjustment);
var popoverLocation=showBiggerImmersiveView?{left:0,top:0}:{left:20,top:28};
return{localContent:"#IV-Main",width:popoverWidth,location:function(popover,settings){return popoverLocation;
...
}var popoverTop=($("body").scrollTop()+28)+"px";
if(showBiggerImmersiveView){$("body").css("overflow","hidden");
popoverTop=$(document).scrollTop()+"px";
$(".ap_closetext").css("font-size","16px");
}$("#IV-Main").parents(".ap_popover").css({"top":popoverTop});
selectThumbnail();
resizeIV();
...
var lastClickedThumbnailEvent;
var thumbnailSelectionInitialized=false;
...
resizeMainBlockItem();
resizeHeroImage();
}function gerHeroDimParams(imageSize){if(isBurj()){if(!imageSize||typeof imageSize=="undefined"){var imageHash=getHeroImageHash(currentItem.color);
if(imageHash==-1){return;
}var keys=$.map(imageHash.main,function(value,key){return key;
});
var imageUrl=keys[0];
imageSize=imageHash.main[imageUrl];
}var dims=getHeroDims(imageSize);
var xPos=getXPosForHeroImage(dims[0],imageSize);
var yPos=75;
return{"height":dims[1],"width":dims[0],"xPos":xPos,"yPos":yPos};
}}function resizeHeroImage(){if(!heroContainer.length){return;
}if(isBurj()){var heroDimParams=gerHeroDimParams();
if(heroDimParams&&typeof heroDimParams!="undefined"){if(heroDimParams.xPos&&typeof heroDimParams.xPos!="undefined"){heroContainer.css({"background-position":heroDimParams.xPos+"px "+heroDimParams.yPos+"px"});
}else{heroContainer.css({"background-position":"center "+heroDimParams.yPos+"px"});
}heroContainer.css({"background-size":heroDimParams.width+"px "+heroDimParams.height+"px","max-width":heroDimParams.width+"px","min-height":heroDimParams.height+"px","filter":"","-ms-filter":""});
}}}function resizeMainBlockItem(){var imgWrappers=canvas.find(".item .imgTagWrapper");
var mainBlockHeight=utils.getMainBlockHeight();
var maxDims=utils.getMaxDimensionsOfMainImage();
...
}if(maxDims&&typeof maxDims!=="undefined"){maxWidth=Math.min(maxDims[0],maxWidth);
maxHeight=Math.min(maxDims[1],maxHeight);
...
return item;
}function createIngress(type,data,index,thumbnailIndex,isFirstInteraction){var item=thumbs.find(".altIngressTemplate").clone().removeClass("altIngressTemplate").removeClass("aok-hidden").addClass("item");
var thumb=data+"+";
item.find("#altIngressText").append(thumb);
item.find(".a-declarative").attr("data-thumb-action",'{"index":"'+index+'", "thumbnailIndex":"'+thumbnailIndex+'", "type": "'+type+'", "variant": ""}');
return item;
}function createAndAddNewThumbnails(newColor,isFirstInteraction,ajaxCall){thumbs.find(".item").remove();
...
var totalAlts=colorImages[currentItem.color].length+videos.length;
if(options.useAltIngress&&options.maxAlts<totalAlts){if(!ajaxCall||colorImages[currentItem.color].length<=options.maxAlts){P.when("postMetric").execute(function(postMetricClass){postMetricClass.postCountMetric("ccx-altIngress-shown");
});
}thumbs.append(createIngress("ingress",totalAlts-options.maxAlts,0,thumbnailIndex));
...
}if(options.burjImageBlock){selectThumbnail(lastClickedThumbnailEvent);
...
if(eventData.type==="ingress"){if(event.type==="click"){P.when("postMetric").execute(function(postMetricClass){postMetricClass.postCountMetric("ccx-altIngress-clicked");
});
openImmersiveView(event,true);
}return -1;
...
if(!options.burjImageBlock||options.burjImageBlock&&event.type==="click"){thumbs.find(".a-button").removeClass("a-button-selected");
...
}if(options.burjImageBlock&&event.type==="click"||options.burjImageBlock&&!thumbnailSelectionInitialized){lastClickedThumbnailEvent=event;
thumbnailSelectionInitialized=true;
...
}function hideHeroImage(){if(heroContainer.length){heroContainer.addClass("hide-background");
...
if(heroContainer.length&&currentItem.type!=="hero-image"){heroContainer.addClass("twisted");
...
}function getXPosForHeroImage(width,imageSize){var heroPos;
if(isBurj()){var vpWidth=getWidth();
var center=options.heroFocalPoint;
var ppdMaxWidth=1500;
var ppdMinWidth=1024;
var actionPanelWidth=420;
var actionPanelPadding=23;
if(center&&typeof center!="undefined"){var offset=Math.max(((vpWidth-Math.min(ppdMaxWidth,width))/2),0);
var imageOffset=Math.max((vpWidth-width)/2,0);
var offsetDiff=offset-imageOffset;
vpWidth=Math.max(Math.min(vpWidth,ppdMaxWidth),ppdMinWidth);
var leftColCenter=(vpWidth-actionPanelWidth-actionPanelPadding)/2;
heroPos=offsetDiff+leftColCenter-((width/imageSize[0])*center);
}}return heroPos;
}function getHeroDims(imageSize){var dims;
if(isBurj()){var heroResizingMaxCutoff=1280;
var heroResizingMinCutoff=1024;
var width=imageSize[0];
var height=imageSize[1];
var viewPortHeight=getHeight();
if(viewPortHeight<=768){width=0.7*width;
height=0.7*height;
}else{var vpWidth=getWidth();
var scalingFactorAddr=(vpWidth-heroResizingMinCutoff)/(heroResizingMaxCutoff-heroResizingMinCutoff);
scalingFactorAddr=Math.max(Math.min(scalingFactorAddr,1),0);
width=(0.7+0.3*scalingFactorAddr)*width;
height=(0.7+0.3*scalingFactorAddr)*height;
}dims=[width,height];
}return dims;
...
}function updateHeroContainer(imageUrl,imageSize){if(!heroContainer.length){return;
}if(!imageUrl){return;
}if(!imageSize){return;
}var width=imageSize[0];
var height=imageSize[1];
heroContainer.css({"background-image":'url("'+imageUrl+'")'});
if(isBurj()){var heroDimParams=gerHeroDimParams(imageSize);
if(heroDimParams&&typeof heroDimParams!="undefined"){width=heroDimParams.width;
height=heroDimParams.height;
if(heroDimParams.xPos&&typeof heroDimParams.xPos!="undefined"){heroContainer.css({"background-position":heroDimParams.xPos+"px "+heroDimParams.yPos+"px"});
}else{heroContainer.css({"background-position":"center "+heroDimParams.yPos+"px"});
}}}heroContainer.css({"background-size":width+"px "+height+"px","max-width":width+"px","min-height":height+"px","filter":"","-ms-filter":""});
}function isBurj(){return options.burjImageBlock;
...
if(maxDims&&typeof maxDims!=="undefined"){holderRatio=itemContainer.width()/itemContainer.height();
...
return image;
}function mainImageClick(event){if($("input#show-immersive-view-on-softlines").val()=="true"){var immersiveViewCounter="SoftlinesImmersiveViewOnPC";
ue.count(immersiveViewCounter,(ue.count(immersiveViewCounter)||0)+1);
}openImmersiveView(event);
}function openImmersiveView(e,bypassUseIV){if(!options.useIV&&!bypassUseIV){return;
}if(airyInstance){airyConfig.volume=airyInstance.getVolume();
airyInstance.pause();
}immersiveView.iVController(e,options,utils,logger,bypassUseIV);
}function destroyMagnifier(){if(magnifier){magnifier.destroy();
magnifier=undefined;
}}function loadMagnifier(){if(!options.useHoverZoom){return;
}var logMetrics=null;
P.when("imageMagnifier").execute(function(im){var opts={adjustment:[0,0,1,1],generalImageUrls:options.staticImages,location:"over",minHeightElement:"#imageBlock",preload:"ready",zIndex:190,useHoverZoomIpad:options.useHoverZoomIpad,useHoverZoom:options.useHoverZoom,logMetrics:logMetrics,isUDP:options.isUDP,burjImageBlock:options.burjImageBlock,useClickZoom:options.useClickZoom,zoomOutIcon:options.staticImages.zoomOutCur,isHoverZoomShawdowEnabled:options.isHoverZoomShawdowEnabled,isHoverZoomEnhancedShadowEnabled:options.isHoverZoomEnhancedShadowEnabled};
destroyMagnifier();
...
"use strict";
P.now().register("postMetric",function(){var postMetricClass={postCountMetric:function(metricName){if(typeof metricName!=="undefined"){var ue=window.ue;
if(ue){var existingValue=ue.count(metricName)||0;
ue.count(metricName,existingValue+1);
}}}};
return postMetricClass;
});
